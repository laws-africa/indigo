...
class BaseBulkCreator(LocaleBasedMatcher):
    ...
    def validate_row(self, row):
        ...
        # if commencement_date/commenced_on_date is set to any day in the year 9999, clear it
        if (getattr(row, 'commencement_date') and getattr(row, 'commencement_date').year == 9999) or \
                (getattr(row, 'commenced_on_date') and getattr(row, 'commenced_on_date').year == 9999):
            row.commencement_date = None

        if self.dry_run:
            if not row.commenced:
                row.notes.append('Uncommenced')
            elif row.commenced and not row.commencement_date:
