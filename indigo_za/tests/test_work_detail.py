...
class WorkDetailZATestCase(TestCase):
    ...
    def test_numbered_title_bylaw(self):
        work = Work.objects.create(frbr_uri='/akn/za-cpt/act/by-law/2020/problem-property', country=self.country)
        self.assertIsNone(self.plugin.work_numbered_title(work))

    def test_numbered_title_ministerial_order(self):
        work = Work.objects.create(frbr_uri='/akn/za/act/mo/2020/acsa-covid-19', country=self.country)
        self.assertIsNone(self.plugin.work_numbered_title(work))

    def test_numbered_title_rules(self):
        work = Work.objects.create(frbr_uri='/akn/za/act/rules/2016/national-assembly', country=self.country)
        self.assertIsNone(self.plugin.work_numbered_title(work))
